...
export const PendingTable = () => {
  ...
  const records = requests
    .filter((request) => request.oneHasApproved && !request.twoHasApproved)
...
export const AllTable = () => {
  ...
  const firstIndex = lastIndex - recordsPerPage;
  const records = requests
    .filter((request) => request.twoHasApproved)
    .slice(firstIndex, lastIndex);
  const npages = Math.ceil(requests.length / recordsPerPage);
  const numbers = [...Array(npages + 1).keys()].slice(1);
  ...
  const getApprovedRequests = async () => {
    ...
    try {
      await axios
        .get(
          `${apiURL}/GetAllAccessRequestByApprovalNEmail?staffEmail=${email}@premiumtrustbank.com&statusCode=0002`
        )
        .then((response) => {
          console.log(response.data, "Approved Request");
          setRequests(response.data.data);
        });
    } catch (error) {
      console.error("Error fetching access requests:", error);
      ...
      <CardHeader floated={false} shadow={false} className="rounded-none">
        <div className="mb-2 flex flex-col justify-between gap-8 md:flex-row md:items-center">
          <div>
            ...
            <Typography color="gray" className="mt-1 font-normal">
              These are details of requests that has been approved
            </Typography>
          </div>
        </div>
      </CardHeader>
